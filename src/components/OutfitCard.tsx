...
export const OutfitCard: React.FC<OutfitCardProps> = ({
  ...
  const ItemRow: React.FC<{ 
    ...
    labelColor?: string;
    isCore?: boolean;
  }> = ({ label, value, labelColor = 'text-slate-500', isCore = false }) => {
  ...
  const coreItems = [
    { label: 'Jacket', value: outfit.jacket?.name },
    { label: 'Shirt', value: outfit.shirt?.name },
    { label: 'Pants', value: outfit.pants?.name },
    { label: 'Shoes', value: outfit.shoes?.name }
  ];

  const accessories = [
    { label: 'Belt', value: outfit.belt?.name },
    { label: 'Watch', value: outfit.watch?.name },
    { label: 'Style', value: outfit.tuck }
  ];
      ...
      >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="space-y-4">
            ...
            <ItemRow label="Belt" value={outfit.belt?.name} labelColor="text-blue-600" />
            <ItemRow label="Watch" value={outfit.watch?.name} labelColor="text-blue-600" />
            <ItemRow label="Style" value={outfit.tuck} labelColor="text-green-600" />
          </div>
        </div>
      </div>
    );
